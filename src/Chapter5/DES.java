...
public class DES {
    ...
    //选择压缩变换S盒代替
    private long S(long r) {
        int len_input = 6;
        int len_output = 4;
        int turn = 1; //轮数
        int total_turn = 8; //总轮数
        long newR = 0;
        for (int[][] s: S) {
            long input = (r >>> ((total_turn-turn)*len_input)) & 0x3F; //取六位
            int num1 = (int)( (input>>>(len_input-1))*2 + (input&0x1) ); //首尾两位
            int num2 = (int)( input & 0x1E )/2; //中间四位
            newR = (newR<<len_output) + s[num1][num2];
            turn++;
        }
        return newR;
    }
    ...
    //置换运算P
    private long P(long r) {
        int len_input = 32;
        int len_output = 32;
        long newR = 0;
        for(int i=0; i<len_output; i++) {
            long num = (r>>>(len_input-P[i]))&1;
            newR = newR * 2 + num;
        }
        return newR;
    }
